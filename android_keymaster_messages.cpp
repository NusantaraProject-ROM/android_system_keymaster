...
size_t ImportWrappedKeyRequest::SerializedSize() const {
    return sizeof(uint32_t) /* wrapped_key_data_length */ + wrapped_key_data_length +
           sizeof(uint32_t) /* wrapping_key_data_length */ + wrapping_key_data_length;
}

uint8_t* ImportWrappedKeyRequest::Serialize(uint8_t* buf, const uint8_t* end) const {
    buf = append_size_and_data_to_buf(buf, end, wrapping_key_data, wrapping_key_data_length);
    return append_size_and_data_to_buf(buf, end, wrapped_key_data, wrapping_key_data_length);
}
...
bool ImportWrappedKeyRequest::Deserialize(const uint8_t** buf_ptr, const uint8_t* end) {
    ...
    UniquePtr<uint8_t[]> deserialized_wrapping_key_material;
    UniquePtr<uint8_t[]> deserialized_wrapped_key_material;
    if (!copy_size_and_data_from_buf(buf_ptr, end, &wrapping_key_data_length,
                                     &deserialized_wrapping_key_material) ||
        !copy_size_and_data_from_buf(buf_ptr, end, &wrapped_key_data_length,
                                     &deserialized_wrapped_key_material))
        return false;
    wrapping_key_data = deserialized_wrapping_key_material.release();
    wrapped_key_data = deserialized_wrapped_key_material.release();
    return true;
}
